...
db = client['crypto']
collection = db['crypto_table_new']
...
def charts():
    ...
    # Fetch data for bar chart (Least 5 cryptocurrencies by minimum price)
    least_cryptos_data = list(collection.aggregate([
        {
            "$group": {
                "_id": "$symbol",
                "minPrice": {"$min": "$price"}
        ...
        {"$sort": {"minPrice": 1}},  # Sort in ascending order
        {"$limit": 5}
    ]))

    least_cryptos_data = [{'symbol': entry['_id'], 'price': entry['minPrice']} for entry in least_cryptos_data]

    return render_template('charts.html', bitcoin_data=bitcoin_data, top_cryptos_data=top_cryptos_data, least_cryptos_data=least_cryptos_data)
